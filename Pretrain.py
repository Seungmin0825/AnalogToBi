...
# Adding Circuit Type tokens for conditional generation
circuit_type_tokens = [
...
# Print the results
print("Devices in order: [", devices[:5], "...", devices[-20:], "]")
print("Vocabulary size:", len(devices))
print("Device to index mapping:", {k: v for k, v in list(stoi.items())[:5]}, "... (truncated)")
print("Index to device mapping:", {k: v for k, v in list(itos.items())[:5]}, "... (truncated)")

encode = lambda s: [stoi[c] for c in s] # encoder: take a string, output a list of integers
decode = lambda l: '->'.join([itos[i] for i in l]) + '->'

print("Loading training data...")
train_data = np.load(Trainingdata, allow_pickle=True)
print(f"Training data shape: {train_data.shape}")

# Convert object array to regular array for efficient processing
if train_data.dtype == object:
    # Flatten all sequences into one long list for batch encoding
    all_tokens = []
    sequence_lengths = []
    ...
    # Encode all tokens at once (much faster)
    encoded_tokens = encode(all_tokens)
    
    # Reconstruct sequences
    encoded_train = []
    start_idx = 0
    for length in sequence_lengths:
        encoded_train.append(encoded_tokens[start_idx:start_idx + length])
        start_idx += length
    
    train_data = torch.tensor(encoded_train, dtype=torch.long)
else:
    # Original fast method for regular arrays
    train_data = torch.tensor(encode(train_data.flatten()), dtype=torch.long).view(train_data.shape)

print("Loading validation data...")
val_data = np.load(Validationdata, allow_pickle=True)
print(f"Validation data shape: {val_data.shape}")

# Convert object array to regular array for efficient processing
if val_data.dtype == object:
    # Flatten all sequences into one long list for batch encoding
    all_tokens = []
    sequence_lengths = []
    ...
    start_idx = 0
    for length in sequence_lengths:
        encoded_val.append(encoded_tokens[start_idx:start_idx + length])
        start_idx += length
